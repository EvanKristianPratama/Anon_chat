...
import { sanitizeAvatarUrl, type AvatarPreference } from "@/lib/avatar";
...
const normalizeAvatar = (raw: unknown): string | undefined => {
...
export interface UseChatSocketReturn {
  ...
  messages: Message[];
  joinQueue: (alias: string, avatar?: AvatarPreference) => void;
  skip: () => void;
...
export function useChatSocket(): UseChatSocketReturn {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      switch (envelope.event) {
        ...
        case "room:matched": {
          ...
          setPartnerAvatar(normalizeAvatar(payload.partnerAvatar) ?? null);
          pushSystem(
            setMessages,
        ...
        case "chat:text": {
          ...
          appendMessage(setMessages, {
            ...
            text: payload.text,
            avatarUrl: normalizeAvatar(payload.avatar)
          });
          return;
        }
        case "chat:image": {
  ...
  const joinQueue = useCallback(
    (alias: string, avatar?: AvatarPreference) => {
      sendEvent("queue:join", avatar ? { alias, avatar } : { alias });
    },
    [sendEvent]
  );
